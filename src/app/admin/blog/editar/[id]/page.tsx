...
export default function EditarBlogPostPage({ params }: EditarPostParams) {
  ...
  const [formData, setFormData] = useState({
    ...
    imagem_destaque_url: '',
    is_published: false,
    like_count: 0, // Adicionado para exibir
    view_count: 0, // Adicionado para exibir
  });
      ...
      try {
        ...
        if (post) {
          setFormData({
            ...
            like_count: post.like_count || 0, // Carregar like_count
            view_count: post.view_count || 0, // Carregar view_count
          });
  ...
  }, [postId, router]); // Adicionado router como dependência por segurança
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (type === 'checkbox') {
      ...
      if (name === 'titulo' && formData.slug === generateSlug(formData.titulo)) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Não incluir like_count e view_count no payload para updatePost
    const { like_count, view_count, ...payload } = formData;
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="flex justify-end space-x-4">
          ...
          <button type="submit" disabled={isLoading} className={`px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white ${isLoading ? 'bg-purple-400' : 'bg-purple-600 hover:bg-purple-700'} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500`}>
            {isLoading ? (
